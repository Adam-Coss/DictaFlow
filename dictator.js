...
function applyPreset(name) {
  const presets = {
    ...
    normal: { sentenceSpeed: 1.2, wordSpeed: 1.0,  slowWordSpeed: 0.8, auxSpeed: 1.2, wordPause: 1500, longWordThreshold: 9 },
    fast:   {
      sentenceSpeed: 1.5,
      wordSpeed: 1.0,
      slowWordSpeed: 1.0,
  ...
  if (typeof p.voice !== 'undefined') { voiceSelect.value = p.voice; }
  if (typeof p.nightMode === 'boolean') {
    themeToggle.checked = p.nightMode;
    document.body.classList.toggle('night-mode', p.nightMode);
  }
  if (typeof p.textSize !== 'undefined') {
    textSizeControl.value = String(p.textSize);
    textSizeValue.textContent = p.textSize + 'px';
    textInput.style.fontSize = p.textSize + 'px';
  }
  if (typeof p.dictationSize !== 'undefined') {
    dictationSizeControl.value = String(p.dictationSize);
    dictationSizeValue.textContent = p.dictationSize + 'px';
    sentenceContent.style.fontSize = p.dictationSize + 'px';
  }
  if (typeof p.speakPunctuation === 'boolean' && speakPunctToggle) { speakPunctToggle.checked = p.speakPunctuation; }
  if (typeof p.speakSymbols === 'boolean' && speakSymbolsToggle) { speakSymbolsToggle.checked = p.speakSymbols; }
...
const letterSounds = {
  ...
  'ж': 'же', 'з': 'зэ', 'и': 'и', 'й': 'и краткое', 'к': 'кэ', 'л': 'лэ',
  'м': 'мэ', 'н': 'нэ', 'о': 'о', 'п': 'пэ', 'р': 'рэ', 'с': 'сэ', 'т': 'тэ',
  'у': 'у', 'ф': 'фэ', 'х': 'хэ', 'ц': 'цэ', 'ч': 'че', 'ш': 'шэ', 'щ': 'ще',
...
function resetSettings() {
  ...
  setCookie("dictisSettings", "", -1);
  sentenceSpeedControl.value = "1.2";    sentenceSpeedValue.textContent = "1.2x";
  wordSpeedControl.value = "1";         wordSpeedValue.textContent = "1x";
  slowWordSpeedControl.value = "0.8";   slowWordSpeedValue.textContent = "0.8x";
  auxSpeedControl.value = "1.2";        auxSpeedValue.textContent = "1.2x";
  longWordThresholdControl.value = "9"; longWordThresholdValue.textContent = "9";
  wordPauseControl.value = "1500";      wordPauseValue.textContent = "1500";
  voiceSelect.value = "zahar";
  textSizeControl.value = "12"; textSizeValue.textContent = "12px"; textInput.style.fontSize = "12px";
  dictationSizeControl.value = "24"; dictationSizeValue.textContent = "24px"; sentenceContent.style.fontSize = "24px";
  themeToggle.checked = false; document.body.classList.remove('night-mode');
